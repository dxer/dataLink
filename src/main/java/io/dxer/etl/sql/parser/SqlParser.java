...
import io.dxer.etl.sql.tree.Statement;
...
import io.dxer.etl.sql.tree.Expression;
import io.dxer.etl.sql.tree.Node;
...
public class SqlParser {
    ...
    public Statement createStatement(String sql, ParsingOptions parsingOptions) {
        Function<SqlBaseParser, ParserRuleContext> parseFunction = new Function<SqlBaseParser, ParserRuleContext>() {
            @Nullable
            @Override
            public ParserRuleContext apply(@Nullable SqlBaseParser sqlBaseParser) {
                return sqlBaseParser.singleStatement();
            }
        };
        return (Statement) invokeParser("statement", sql, parseFunction, parsingOptions);
    }
    ...
    public Expression createExpression(String expression, ParsingOptions parsingOptions) {
        Function<SqlBaseParser, ParserRuleContext> parseFunction = new Function<SqlBaseParser, ParserRuleContext>() {
            ...
            @Override
            public ParserRuleContext apply(@Nullable SqlBaseParser sqlBaseParser) {
                return sqlBaseParser.singleExpression();
            }
        };

        return (Expression) invokeParser("expression", expression, parseFunction, parsingOptions);
    }
