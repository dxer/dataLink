...
class DataFrameFunctions(data: DataFrame) extends Logging with Serializable {
  ...
  def insertIntoHBase(tableName: String, options: Map[String, String] = new HashMap[String, String]): Unit = {
    ...
    var rowKeyField = options.getOrElse("hbase.table.rowkey.field", "")
    ...
    val rdd = if (options.getOrElse("bulkload.enable", "true").toBoolean &&
      options.getOrElse("bulkload.autoSort", "false").toBoolean) {
    ...
    if (options.getOrElse("hbase.check.table", "true").toBoolean) {
    ...
    options.getOrElse("bulkload.enable", "true").toBoolean match {
      // bulk load mode
      case true =>
